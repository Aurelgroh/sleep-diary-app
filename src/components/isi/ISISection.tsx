  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Insomnia Severity Index (ISI)</h2>
        ...
        {!showForm && (
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition text-sm focus-ring min-h-[44px]"
          >
      ...
      {showForm && (
        <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
          <h3 className="font-medium text-slate-900 dark:text-slate-100 mb-4">New ISI Assessment</h3>
          <ISIEntryForm
            patientId={patientId}
      ...
      <div className="p-6">
        {scores.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm">No ISI assessments recorded</p>
            <p className="text-slate-500 dark:text-slate-500 text-xs mt-1">Click &quot;Add ISI Assessment&quot; to record one</p>
          </div>
        ) : (
          <div className="space-y-3" role="list" aria-label="ISI assessment history">
            {scores.map((score) => (
              <div
                key={score.id}
                role="listitem"
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">{formatAssessmentType(score.assessment_type)}</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{score.score}</p>
