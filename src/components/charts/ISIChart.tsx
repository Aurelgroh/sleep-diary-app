  ...
  if (scores.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-slate-500">No ISI scores recorded yet</p>
        <p className="text-sm text-slate-400 mt-1">ISI assessments track insomnia severity over time</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h3 className="font-medium text-slate-900">Insomnia Severity Index (ISI) Over Time</h3>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        {SEVERITY_BANDS.map(band => (
          <div key={band.label} className="flex items-center gap-1.5">
            ...
            <span className="text-slate-600">{band.label} ({band.min}-{band.max})</span>
          </div>
        ))}
      </div>
              ...
              content={({ active, payload, label }) => {
                ...
                return (
                  <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3">
                    ...
                    <p className="text-sm capitalize text-slate-500 mb-2">{type?.replace('_', ' ')}</p>
                    <p className="text-sm flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: getSeverityColor(score) }} />
                      <span className="text-slate-600">Score:</span>
                      <span className="font-bold">{score}</span>
                      <span className="text-slate-500">({getSeverityLabel(score)})</span>
                    </p>
                  </div>
                )
              }}
