...
export function PatientChatSection({ patientId }: PatientChatSectionProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`unread:patient:${patientId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          if (payload.new && (payload.new as { sender_type: string }).sender_type === 'therapist') {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Chat with your therapist</p>
          </div>
        </div>
        <svg
